...
写节点时建议：**单一职责**（一个节点只做一件事，方便维护和复用）、**纯函数优先**（尽量不依赖外部可变状态，相同输入得到相同输出，便于测试和重试）。常见用法包括在节点里做 **LLM 调用**、**工具调用**，或组合成 Agent 的一步。
...
Edge 定义节点之间的连接与执行顺序；一个节点可有多个出边，多节点可指向同一节点。可以这样理解：**边就是流程的「连接线」**——从哪个节点到哪个节点、先执行谁后执行谁，都由边来决定。边的两大基础类型是 **普通边**（固定从 A 到 B）和 **条件边**（根据当前状态决定下一步走哪条路）；它们的核心作用都是 **实现复杂的路由逻辑**，让工作流既能顺序执行，也能按条件分支、汇聚。类型包括：普通边、条件边、入口点、条件入口点（后两种见下文）。
...
- **普通边**：不判断条件，直接「从当前节点到下一个节点」。
- **条件边**：先执行一个路由函数，根据返回值决定下一步走到哪个（或哪些）节点。
- **入口点**：指定「用户输入进来后，图从哪个节点开始跑」，即图的起点。
- **条件入口点**：用户输入进来后，先调一个函数，根据结果决定从哪个（或哪些）节点开始执行（起点也可动态选）。
