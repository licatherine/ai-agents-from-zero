...
**学习建议：** 选型时可先问三句话：**缺的是「知识」还是「行为」？** 缺知识（模型不知道的内容、私有文档、最新信息）→ 优先 RAG；缺行为（不会按格式输出、话术不统一、指令遵循差）→ 考虑微调。**要不要自动查资料、调 API、执行多步？** 要 → 用智能体。本篇把 RAG、微调、续训、智能体串起来，便于你在实际场景里做大致选型。
...
1. RAG 和微调在“补知识”与“改行为”上的分工差异是什么？
   ...
   **答案：** RAG 主要解决“模型不知道最新或私有知识”的问题，是运行时补上下文；微调主要解决“模型回答风格、格式、行为不稳定”的问题，是通过训练改行为模式。简单说，RAG 更偏补知识，微调更偏改习惯。

2. 为什么说续训的硬件成本通常远高于微调？

   **答案：** 因为续训通常要在更大规模语料上继续训练模型参数，涉及更长训练时长、更高显存、更复杂的数据清洗和训练稳定性问题。微调一般只在较小数据集上做有针对性的调整，算力和成本都低很多。

3. MCP、Function Calling、Workflow 在“接能力”和“控流程”上分别更偏哪一层？

   **答案：** MCP 更偏标准化接入层，它解决“外部能力如何被统一暴露和复用”；Function Calling 更偏模型调用层，它解决“模型如何选择函数并生成参数”；Workflow 更偏流程编排层，它解决“步骤是否固定、顺序如何控制”。三者不是替代关系，而是协议、调用、流程三层分工。

4. 现在请你回过头，用这章的框架重新解释“五大模块到底怎么选”，并说明什么时候该用 Workflow，什么时候才该上 Agent。

   **答案：** 五大模块可以按问题类型来选：能靠提问方式解决的先上 Prompt，缺外部知识上 RAG，回答风格和格式长期不稳再看微调，模型底层领域知识大面积缺失才考虑续训；如果步骤固定、顺序明确、强调稳定性，优先用 Workflow；只有当任务需要临场决策、动态选工具、边做边调整时，才更适合上 Agent。

5. 如果你要给团队新人做一次技术路线分享，你会如何用“知识、行为、流程”三条线来组织这些方案？

   **答案：** 我会按三条线来讲：知识线看是否缺外部资料，优先 RAG 或续训；行为线看是否需要稳定格式、口吻或偏好，优先 Prompt 或微调；流程线再分成两类，固定流程优先 Workflow，动态决策和工具编排才考虑 Agent。这样团队就能先识别问题类型，再选最小够用的方案。

**本章小结：**

- **RAG** 解决的是“模型不知道最新或私有知识”，本质是运行时检索外部资料再生成，不会改模型参数；它和微调最大的边界，是“补知识”而不是“改行为”。
- **微调** 更适合稳定输出风格、格式、偏好或特定任务行为；常见手段包括 LoRA、QLoRA，但仍要关注数据质量、过拟合和灾难性遗忘。
- **续训** 适合领域语料量大、底层知识分布确实和通用模型差异很大的场景；它的成本和风险通常高于微调，多数项目不会把它作为第一选择。
- **智能体 / Workflow** 解决的是流程问题：步骤固定、强调稳定和可解释时优先 Workflow；需要动态拆任务、选工具、连续决策时才更适合 Agent。Function Calling 偏模型调用机制，MCP 偏外部能力的标准化接入协议。
- 从选型顺序看，比较稳的思路通常是：**先 Prompt，再看 RAG，再看微调，续训放到更后面；流程固定先 Workflow，流程开放才上 Agent。**
- 从掌握结果看，学完本章后，你至少应该：能区分 RAG、微调、续训、Workflow、Agent 分别最适合解决什么问题；能用“缺知识 / 缺行为 / 缺流程能力”这条主线做初步选型；能理解 Function Calling、MCP、Workflow 在智能体体系里的大致位置。

**建议下一步：** 如果你想先亲手体验“给模型补知识”，就进入 [2-RAG-搭建企业私有&个人知识库](2-RAG-搭建企业私有&个人知识库.md)；如果你想先做一个能跑起来的智能体体验，则继续看 [3-基于 Coze&Dify 平台的智能体开发](3-基于Coze&Dify平台的智能体开发.md)。等你进入代码主线时，再去看 [9-LangChain 概述与架构](9-LangChain概述与架构.md)，会更容易把这些概念落到代码实现上。
