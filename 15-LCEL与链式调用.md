...
- **在本章中**：**Runnable** 就是这样一个抽象基类；Prompt 模板、LLM、Output Parser、以及用 `|` 拼出来的链，都是「实现了 Runnable 接口」的具体类型，因此都可以用同一套 `invoke`、`stream`、`batch` 等方式调用。
- **LangChain 中常见的抽象基类**（均位于 `langchain_core`）：**Runnable**（可执行组件的统一接口，本章核心）；**BaseChatModel** / **BaseLLM**（聊天模型 / 语言模型的基类，如 ChatOpenAI 的父类）；**BasePromptTemplate**、**BaseChatPromptTemplate**（提示词模板的基类）；**BaseOutputParser**（输出解析器的基类，如 PydanticOutputParser 的父类）。这些基类大多实现了 Runnable 接口，因此既能各自约定「模型该长什么样」「解析器该长什么样」，又能被 LCEL 用 `|` 统一组合。
...
- **使用后**：工具也实现 Runnable，用 `tool.invoke(input)`，和 prompt、model、parser 用法一致，可直接用 `|` 参与组合。
...
- **串行链**：先检索再生成（RAG 简化版）、先总结再翻译、多步推理链。
