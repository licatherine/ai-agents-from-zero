...
Runnable 表示「一个可以调用（运行）的流程单元」，可以是：

- 单个组件（如 prompt、model）
- 一个序列流程（如 prompt → model → parser）
- 并行、多路、多输入多输出的复合结构

只要实现了 Runnable 接口，就可以像函数一样 `.invoke(input)`，或用管道符 `|` 与其他 Runnable 组合。
...
**LCEL**（LangChain Expression Language，LangChain 表达式语言）是专门用于**组合 Runnable 组件**的声明式语法，其**核心操作符是管道符 `|`**。
...
![顺序链数据流：提示 → 模型 → 解析器](images/15/image105.jpeg)

> **说明**：图中从左到右依次为提示模板、模型、输出解析器，对应 LCEL 写法 `chain = prompt | model | parser`。
