"""
【案例】接入大模型的最小对话图：用户消息 → 模型节点 → 回复，演示 State 中使用 Annotated[List, add_messages] 做消息列表追加，以及图的构建与执行。

对应教程章节：第 22 章 - LangGraph 概述与快速入门 → 2、HelloWorld 快速入门

知识点速览：
- add_messages：LangGraph 内置的「规约器」（Reducer），从 langgraph.graph.message 导入。用于 State 中的消息列表字段时，表示「追加」而非「覆盖」——节点只返回新增消息，框架自动合并到列表末尾，适合多轮对话。
- messages 字段用 Annotated[List, add_messages]：节点只需 return {"messages": [新消息]}，历史消息不会丢失。
- 节点中调用 llm.invoke(state["messages"])，传入历史消息列表，返回新消息并写回 state。
- 图结构：START → model → END；invoke 时传入初始 messages（HumanMessage 或兼容格式），执行后从 result["messages"] 取最后一条即为模型回复。
- 与第 10 章一致的「调用三件套」：模型名、API Key、Base URL（如阿里百炼 compatible-mode）。
"""
