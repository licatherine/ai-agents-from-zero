...
![LangGraph 流式传输概念：从图执行到实时输出多种数据](images/25/image62.jpeg)
...
![stream_mode 选项与含义示意](images/25/image63.jpeg)
...
![流式传输状态：updates 与 values 的区别](images/25/image64.jpeg)
...
![多模式流式传输：同时使用 values、updates、debug 等](images/25/image65.jpeg)
...
![messages 模式：逐 token 流式输出 LLM 结果](images/25/image67.jpeg)
...
![自定义数据流：在节点内通过 get_stream_writer 发送自定义消息](images/25/image68.jpeg)
...
![状态持久化：检查点与 thread_id 的关系](images/25/image70.jpeg)
...
![持久化后端选型：从开发到生产（内存 → SQLite → Postgres/Redis）](images/25/image71.gif)
...
![时间回溯：从历史检查点恢复并可选修改状态后继续执行](images/25/image73.jpeg)
...
![时间回溯伪代码：获取历史、选择检查点、可选更新状态、恢复执行](images/25/image74.jpeg)
...
![子图概念：父图中的一个节点由整张子图实现](images/25/image76.jpeg)
