# =============================================================================
# 标准/工程化写法示例：用 LangChain 调用大模型（LLM）
# =============================================================================
#
# 【这个文件在做什么】
# 用「通义千问 / 阿里云」的接口，通过 LangChain 向大模型发问题，并演示两种拿回答的方式：
#   1. invoke：一次性等模型全部答完再给你结果（适合短问答）
#   2. stream：模型边想边答，像打字机一样一个字一个字出来（适合长文或实时展示）
#
# 【为什么要这样写】
# 把「初始化模型」和「问问题」分开成函数，方便以后在其他地方复用；
# 用 .env 存密钥、用 logging 打日志、用 try/except 区分不同错误，都是写正式项目时的常见做法。
#
# 【运行方式】
# 在项目根目录确保有 .env 文件且配置了 QWEN_API_KEY，然后：python 01-helloworld/StandardDesc.py
#
# =============================================================================
