...
**注意：**由于 github 下载较慢，大家可以将 github 上 coze-studio 镜像下载到 gitee 平台，然后从 gitee 平台下载。

1）在 gitee 平台上：

![](images/6/6-2-2-1.png)

2）复制 Github 上的 coze-studio 地址
...
如果你不想安装 git，你也可以直接下载 github 上 coze 的 zip 包，如下图：
...
`meta.conn_config.api_key`：在线模型服务的 API Key，获取方式见下方获取 Key 和 Model 接入点的 api_key。
`meta.conn_config.model`：在线模型服务的模型 ID。本例中为 Volcengine Ark doubao-seed-1.6 模型接入点的 Endpoint ID，获取方式见下方获取 Key 和 Model 接入点的 model。
...
下图中有一个名叫 docker 的目录，我们要进入到这个项目文件夹中，进行安装。

![](images/6/6-2-2-23.png)

在 coze-studio\docker 当前目录下，测试如下：

**1、输入 docker，点击回车键后，返回下图这样的结果，表明安装是成功的，没有任何问题。**
...
打开安装好的 docker 客户端，正常就可以看到 docker 启动起来了。

![](images/6/6-2-2-28.png)

#### ④ 访问 Coze Studio 界面

安装好后，我们打开浏览器，访问 http://localhost:8888/来打开 Coze Studio ，可以看到如下界面。
...
**1、Go 语言环境：**已安装 Go SDK，且版本为 1.23.4 及以上版本。配置 GOPATH，同时将${GOPATH}/bin 加入到环境变量 PATH 中，保证安装的二进制工具可找到并运行。
...
安装完成后，确保 GO 家目录下的 bin 目录已在环境变量中。
...
这里推荐方式 1，不推荐方式 2，是因为方式 2 下载以后，在 ⑤ 启动服务时，总报错。
...
从 github 下载 coze loop 的 zip 文件到本地，解压即可。
...
1、打开 coze-loop-main\release\deployment\docker-compose 目录下的.env，将 COZE_LOOP_APP_OPENAPI_PORT 更改为`8889`或者其它未被占用的端口。

![](images/6/6-3-2-9.png)

2、打开 coze-loop/release/deployment/docker-compose 目录下的 docker-compose.yml，将${COZE_LOOP_APP_OPENAPI_PORT}的端口更改为`8889`，注意要和.env 中 COZE_LOOP_APP_OPENAPI_PORT 的端口`保持一致`。
...
docker 客户端中，当前容器除了 xxx-init 的镜像启动后又关闭之外，其它的镜像都需要是开启状态的，那就对了。
...
官方 Demo 空间中，我们无权开启自由对比模式，切换到个人空间查看
