...
- 从掌握结果看，学完本章后，你至少应该：能清楚说出 **向量、Embedding、向量数据库** 三者分别是什么，以及它们之间的先后关系；知道为什么“语义相近”可以映射成“向量相近”，以及为什么建库和查询要尽量使用同一个 Embedding 模型；能把“向量化 → 相似度计算 → 写入向量库 → 相似性检索”这条主线和后续 RAG 联系起来，并意识到检索效果不只由向量库本身决定。
