...
**学习建议：** 按文档顺序做即可：发布 → 查 API 文档 → 创建 API Key → 看懂请求格式 → 用 Postman 试一遍（可跳过）→ 用 Python 代码跑通。本地部署 Dify 时，将代码里的 `base_url` 改为 `http://localhost/v1` 即可。
...
![](images/4/4-3-1-1.png)

![](images/4/4-3-1-2.png)

创建后复制即可。

### 3.2 工作流和 API Key 的关系

Dify 的 API 密钥是**和工作流绑定**的，一个 API Key 只能用于**访问特定的工作流**，一个工作流可以对应多个 API Key。

![](images/4/4-3-2-1.png)
...
### 5.1 新建 POST 请求，输入 url

![](images/4/4-5-1-1.png)

### 5.2 添加头信息

![](images/4/4-5-2-1.png)
...
target 字段的值就是我们传递给工作流的输入信息。

### 5.4 发送

![](images/4/4-5-4-1.png)

### 5.5 响应

![](images/4/4-5-5-1.png)

响应开始标志

![](images/4/4-5-5-2.png)

响应结束标志

![](images/4/4-5-5-3.png)

除了开始和结束响应，中间的响应都带有 response body，包含了工作流的运行日志（状态、输入输出等）。最后一个 response body 携带了工作流的最终输出。

![](images/4/4-5-5-4.png)
...
![](images/4/4-6-1-1.png)

第一个就是刚才测试的工作流日志

### 6.2 查看结果

![](images/4/4-6-2-1.png)

### 6.3 详情

![](images/4/4-6-3-1.png)

### 6.4 追踪

![](images/4/4-6-4-1.png)
...
对比 Dify 后台日志和 PyCharm 控制台日志，输出结果内容完全一致

![](images/4/4-7-3-4.png)
...
1. 为什么 Dify 工作流在被 Python 调用之前必须先发布？
   ...
   **答案：** 因为只有发布后的工作流才有稳定可调用的版本和对外 API 能力。未发布的内容还处于编辑状态，接口、节点或参数都可能变化，不适合被外部 Python 服务依赖。

2. `inputs`、`response_mode`、`user` 这三个字段分别解决什么问题？

   **答案：** `inputs` 负责传业务入参，告诉工作流本次处理什么内容；`response_mode` 规定返回方式，是阻塞还是流式；`user` 用来标识调用者，便于做会话区分、审计和平台侧追踪。

3. 为什么流式响应里不能只盯着第一段输出，而要关注最终结束事件？

   **答案：** 因为前几段流式输出只是过程片段，不能代表整次调用真的成功结束。只有看到最终结束事件，才能确认结果是否完整、有没有报错，以及最终该把什么内容交给上层系统。

4. 如果让你把一个 Dify 工作流接进已有 Python 服务，你会如何向团队解释这条调用链的关键步骤？

   **答案：** 可以这样解释：先在 Dify 平台把工作流设计好并发布，再拿到 API Key 和调用地址；Python 侧按约定组织 `inputs` 发请求，按流式或阻塞模式解析返回；最后把结果转成自己服务的响应格式并补上日志和异常处理。

5. 如果 API 调用失败，你会按什么顺序检查：发布状态、API Key、URL、请求体、后台日志？

   **答案：** 我会先看工作流是否真的已发布，再核对 API Key 是否正确、URL 是否指向对的接口，然后检查请求体字段名和数据结构，最后去平台后台日志看节点级报错。这样能最快区分是调用入口问题，还是工作流内部逻辑问题。

**本章小结：**

- **调用前提**：Dify 工作流必须先发布，并创建与之对应的 API Key。
- **调用核心**：最关键的请求要素是 URL、Authorization、`inputs`、`response_mode` 和 `user`；它们共同决定“调哪个工作流、用什么身份、传什么输入、怎么拿结果”。按 Dify 官方接口约定，成功执行后还会返回 `workflow_run_id`、`task_id` 等运行标识，便于后续追踪日志和排查问题。
- **返回模式要分清**：`blocking` 更适合一次性拿最终 JSON 结果，`streaming` 则是基于事件流持续返回执行过程。学这章时，不能只会打印正文，还要知道什么时候该盯住最终结束事件和运行状态。
- **调试主线**：本章最重要的不是记住某段示例代码，而是掌握一条稳定排查路径：**先在平台里发布并调通，再在 Postman / curl 验证请求结构，最后在 Python 里接入，并对照 Dify 日志与本地输出排查问题。**
- 从掌握结果看，学完本章后，你至少应该：知道 Dify 工作流要先发布，才能通过 API 被外部调用；能看懂 Dify API 调用里的核心要素：URL、Authorization、`inputs`、`response_mode`、`user`、运行 ID；能区分 `streaming` 和 `blocking` 两种调用模式，并知道如何从流式结果里抓最终输出和排查异常。

**建议下一步：** 如果你已经跑通 Dify 工作流调用，可以继续学习 [第 5 章 Python 调用 Coze 平台工作流](5-Python调用Coze平台工作流.md)，对比不同平台的 API 调用方式；如果你想进一步做本地化或私有化部署，再看 [第 7 章 Dify 的 Windows 平台部署](7-Dify的Windows平台部署.md) 和 [第 8 章 企业级大模型部署](8-企业级大模型部署.md)。
